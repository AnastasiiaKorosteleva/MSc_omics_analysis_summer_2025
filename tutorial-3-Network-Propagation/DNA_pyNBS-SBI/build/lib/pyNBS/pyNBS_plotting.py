...
# clin_data_fn is the clinical data of TCGA cohort from Broad Firehose
# cluster_assign is a pandas Series of the patient cluster assignments from NBS with patient IDs as the index
# tmax is the maximum plot duration for the KM plot, but the logrank test always calculates to longest survival point
def cluster_KMplot(cluster_assign, clin_data_fn, delimiter='\t', lr_test=True, tmax=-1, verbose=True, **save_args):
    ...
    if lr_test:
        ...
        p = multiv_lr_test(cluster_survivals['overall_survival'], 
                           cluster_survivals[cluster_assign.name], 
                           event_observed=cluster_survivals['vital_status']).p_value
        if verbose:
