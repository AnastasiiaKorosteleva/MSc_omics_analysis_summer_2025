...
def load_binary_mutation_data(filename, filetype='list', delimiter='\t', verbose=True):
    ...
    if filetype == 'list':
        ...
        binary_mat_data = [(line.split(delimiter)[0], line.split(delimiter)[1]) for line in binary_mat_lines]
...
def load_params(params_file=None):
    ...
    if params_file is not None:
        params_file = pd.read_csv(params_file, comment='#', index_col=0)
        params_file.columns = ['value']
        for param in params_file.index:
            if param in run_pyNBS_params:
                if isinstance(run_pyNBS_params[param], bool):
                    run_pyNBS_params[param] = params_file.loc[param, 'value'] == 'True'
                else:
                    run_pyNBS_params[param] = params_file.loc[param, 'value'].astype(type(run_pyNBS_params[param]))
            else:
...
def degree_shuffNet(network, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(f'Note: Maximum number of swap attempts ({edge_len * 10}) exceeded before desired swaps achieved ({edge_len}).')
    if verbose:
        shared_edges = len(set(network.edges).intersection(set(shuff_net.edges)))
        print('Network shuffled:', round(time.time() - shuff_time, 2), 'seconds. Edge similarity:', shared_edges / float(edge_len))
    return shuff_net
...
def label_shuffNet(network, verbose=False):
    ...
    shuff_nodes = network_nodes.copy()
...
def filter_weighted_network(network_file_path, save_path, nodeA_col=0, nodeB_col=1, score_col=2, q=0.9, delimiter='\t', verbose=False):
    ...
    q_score = data.iloc[:, score_col].quantile(q)
    if verbose:
        print(f'{round(q * 100, 2)}% score:', q_score)
    data_filt = data[data.iloc[:, score_col] > q_score].iloc[:, [nodeA_col, nodeB_col, score_col]]
...
def process_TCGA_MAF(maf_file, save_path, filetype='matrix', gene_naming='Symbol', verbose=False):
    ...
    TCGA_sm_mat = TCGA_sm.unstack().fillna(0).astype(int)
    TCGA_sm_mat.index = [pat[:12] for pat in TCGA_sm_mat.index]
    non_dup_IDs = TCGA_sm_mat.index.value_counts()[TCGA_sm_mat.index.value_counts() == 1].index
    if filetype == 'list':
        index_list = TCGA_sm.index
        index_list_filt = [i for i in index_list if i[0][:12] in non_dup_IDs]
        with open(save_path, 'w') as f:
            for sm in index_list_filt:
                f.write(f"{sm[0][:12]}\t{sm[1]}\n")
        if verbose:
            print('Binary somatic mutations list saved')
    else:
        TCGA_sm_mat_filt = TCGA_sm_mat.loc[non_dup_IDs]
        TCGA_sm_mat_filt = TCGA_sm_mat_filt.loc[:, (TCGA_sm_mat_filt > 0).any(axis=0)]
        TCGA_sm_mat_filt.to_csv(save_path)
        if verbose:
